...
class Model(object):
    """
    A bones model.  Any keyword args become members on the model.
    ...
    You can create a model object like this

        p = Person(name='Joe', shoes=11)

    Don't forget you can splat any dictionary as well.

        x = {'name':'Joe', 'shoes':11}
        p = Person(**x)

    In both instances, p is the same.
    """

    def __init__(self, is_meta=False, **kwargs):
        self.is_meta = is_meta  # used to determine if this is an account
                                # specific object or shared at the meta level
        self.update(kwargs)

        #set default values for anything the updates missed
        for name in dir(self):
            if name not in self.__dict__:
                # set a local copy, not just the class level one
        ...
        # seed the doc_type with our class name
...
class BonesModelEncoder(JSONEncoder):
